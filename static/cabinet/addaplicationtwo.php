...
$sql = 'SELECT c.course_price, cn.course_name FROM courses c
		...
		WHERE i.id = ?';
$stmt = $conn->prepare($sql);
$stmt->execute([$_POST['freeinstance']]); 
$stmt->setFetchMode(PDO::FETCH_ASSOC); 
$freecourse = $stmt->fetch();


$stmt->execute([$_POST['paidinstance']]); 
$stmt->setFetchMode(PDO::FETCH_ASSOC); 
$paidcourse = $stmt->fetch();
...
$stmt->execute([$_POST['freeinstance']]); 
$stmt->setFetchMode(PDO::FETCH_ASSOC); 
$freeisregeistered = $stmt->fetch();

$sql = 'SELECT * FROM applications_instances WHERE id_application IN ('.$appstring.') AND id_instance = ?';
$stmt = $conn->prepare($sql);
$stmt->execute([$_POST['paidinstance']]); 
$stmt->setFetchMode(PDO::FETCH_ASSOC); 
$paidisregeistered = $stmt->fetch();
}

if(isset($appstring)) {
	if(empty($freeisregeistered) OR empty($paidisregeistered)) {
...
$token = $appid.'-'.$_POST['freeinstance'].$_POST['paidinstance'].'-'.$client_id.'/'.$_POST['lang'];
$data['token'] = $token;
...
VALUES (?,?)';
$stmt = $conn->prepare($sql);
$stmt->execute([$_POST['freeinstance'], $appid]); 
$stmt->execute([$_POST['paidinstance'], $appid]); 
...
$data['name'] = $freecourse['course_name'].' / '.$paidcourse['course_name'];
$data['message'] = 'ok';
$data['client'] = $_POST["firstname"].' '.$_POST["lastname"];
$data['sum'] = $paidcourse['course_price'];
