...
$sql = 'SELECT * FROM applications WHERE client_id = ? AND id_instance = ?';
$stmt = $conn->prepare($sql);
$stmt->execute([$client_id, $_POST['instance']]); 
$stmt->setFetchMode(PDO::FETCH_ASSOC); 
$isregeistered = $stmt->fetch();

if(empty($isregeistered)) {
	   
$sql = 'INSERT INTO `applications`(`client_id`, `app_date`, `id_instance`, 
`lang`, `amount`) 
VALUES (?,NOW(),?,?,?)';
$stmt = $conn->prepare($sql);
$stmt->execute([$client_id, $_POST['instance'],$_POST["lang"],$course['course_price']]); 
