...
$sql = 'SELECT cn.*,c.course_number,c.course_price FROM college_courses_names cn
		INNER JOIN college_courses c ON c.course_id = cn.course_id
		WHERE cn.lang_id = ?';
...
foreach($courseskz as $key=>$value) {
	$sql = 'SELECT count(id) FROM applicationsf
